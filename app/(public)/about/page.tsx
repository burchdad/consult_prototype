...
export default async function AboutPage() {
  ...
  return (
    <main>
      <Section>
        <h1 className="text-5xl font-black uppercase">{settings?.companyName ?? "Our Company"}: An Industry Leader</h1>
        <div className="mt-8 space-y-4 text-zinc-300">
          <p>{settings?.companyName} is a mission-oriented enterprise delivering technical and operational excellence across federal environments.</p>
          <p>Our teams bring disciplined execution, transparent reporting, and measurable performance outcomes to every engagement.</p>
          <p>From acquisition support to engineering modernization and logistics readiness, we align each workstream to agency mission priorities.</p>
          <p>We are built to scale responsibly with a culture rooted in accountability and stewardship.</p>
        </div>
      </Section>
      ...
      <Section className="grid gap-4 md:grid-cols-2">
        <article className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
          ...
          <p className="mt-3 text-zinc-300">Deliver trusted, secure, and high-performance services that improve federal mission outcomes.</p>
        </article>
        <article className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
          <h2 className="text-3xl font-black uppercase">Vision</h2>
          <p className="mt-3 text-zinc-300">Be the federal partner known for precision execution, innovation, and operational resilience.</p>
        </article>
      </Section>
